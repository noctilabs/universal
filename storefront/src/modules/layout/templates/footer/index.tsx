...
// Fallback content if CMS data is unavailable
const FALLBACK = {
...
export default async function Footer({ locale = "en" }: { locale?: "en" | "es" }) {
  const settings = await getSiteSettings()
  const f = settings?.footer ?? {}

  const contactText = (locale === "es" ? f.contactText_es : f.contactText_en) ?? (locale === "es" ? FALLBACK.contactText_es : FALLBACK.contactText_en)
  const email = f.email ?? FALLBACK.email
  const address = f.address ?? FALLBACK.address
  const instagramUrl = f.instagramUrl ?? FALLBACK.instagramUrl
  const instagramLabel = f.instagramLabel ?? FALLBACK.instagramLabel
  const copyright = (locale === "es" ? f.copyright_es : f.copyright_en) ?? (locale === "es" ? FALLBACK.copyright_es : FALLBACK.copyright_en)
